...
class VisionMamba(nn.Module):
    ...
    def forward_features(self, x, inference_params=None):
        ...
        batch_size = hidden_states.shape[0]  # 8
        height = width = self.img_size // self.patch_size  # 32
        embed_dim = self.embed_dim  # 192
...
def vim_tiny_patch8_224_bimambav2_final_pool_mean_abs_pos_embed_rope_also_residual(
    ...
    model = VisionMamba(patch_size=2, embed_dim=192, depth=4, channels=64, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do", map_location="cpu", check_hash=True
...
# ):
#     model = VisionMamba(
#         patch_size=8, embed_dim=192, depth=24, if_cls_token=True, **kwargs
#     )
#     model.default_cfg = _cfg()
#     if pretrained:
#         checkpoint = torch.hub.load_state_dict_from_url(
...
#     pretrained=False, **kwargs
# ):
#     model = VisionMamba(patch_size=16, embed_dim=768, depth=24, **kwargs)
#     model.default_cfg = _cfg()
#     if pretrained:
#         checkpoint = torch.hub.load_state_dict_from_url(
#             url="to.do", map_location="cpu", check_hash=True
