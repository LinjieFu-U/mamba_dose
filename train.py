...
from vim.mamba_unet import MambaUnet
...
def train():
    ...
    for epoch in range(restore_epoch, FLAGS.max_epoch):
        ...
        if not os.path.exists("./Save/" + FLAGS.DIREC + "/hxdose-mamba-unet"):
            os.makedirs("./Save/" + FLAGS.DIREC + "/hxdose-mamba-unet")
        if (epoch + 1) % 100 == 0:
            torch.save(
                ckpt,
                "./Save/"
    ...
    with torch.no_grad():
        ...
        for data, target,mask,cbct_path,ct_path in tqdm(validloader):
            ...
            for kk in range(len(x_0)):  # x_0 [10,b,1,h,w]
                ...
                if not os.path.exists(
                    "./Test_Output/" + FLAGS.DIREC + "/hxdose-mamba-unet-all"
                ):
                    os.makedirs(
                        "./Test_Output/"
                        + FLAGS.DIREC
                        + "/hxdose-mamba-unet-all"
                    )
